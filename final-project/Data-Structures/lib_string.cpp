...
abj::String::~String(){
  // if(isInitialized==true && this->curr_size != 0 && this->storage != NULL){
    free(this->storage);
    this->storage=NULL;
    this->curr_size=0;
  // }
...
void abj::String::concatenate_at_end(abj::String& data, char separator){
  ...
  this->resize(this->curr_size+data.size()+1);
...
bool abj::String::equals(const abj::String& data) const{
...
bool abj::String::set(int index, char data){
  if(index>=this->curr_size){
    // printf("Can't set(%d,%c) to the string \"%s\". String index out of bound.\n",index,data, this->storage);
    return false;
  }
...
bool abj::String::operator>(const abj::String& data) const{
  char* dataChar = data.get_raw_data();
  if(this->compare_string(this->storage, dataChar) > 0){
    free(dataChar);
    return true;
  }
  free(dataChar);
...
abj::String abj::String::getFileContent(const abj::String& filename){
  char* filenameChar = filename.get_raw_data();
...
printf("Testing String----------------\n");
 ...
 x.capitalize();
 x.insert_char_at_point(x.size()-1,' ');
 x.print();
 ...
 std::vector<abj::String>v;
 for(int i=0; i<10000; i++){
   abj::String newString("Random data is being loaded!");
 ...
 char* nameChar = name.get_raw_data();
 ...
 abj::String answer;
 abj::String space(" ");
 answer = firstPart + space + lastPart;
 answer.capitalize();
 answer.print();
 ...
 abj::String totalSentence = getFileContent(*new abj::String("./Resources/sed-corpus.txt"));
 printf("Size=%d\n",totalSentence.size());
}
