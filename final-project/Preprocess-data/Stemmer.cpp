...
void abj::Stemmer::build_vowel_consonant_array(){
  for(int i=0; i<this->word.size(); i++){
    if(isConsonant(i)) this->vowel_consonant_array.push_back(CONSONANT);
    else if(isPunctuation(this->word.get(i))) this->vowel_consonant_array.push_back(PUNCTUATION);
    else this->vowel_consonant_array.push_back(VOWEL);
  }
}
