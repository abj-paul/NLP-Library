...
MED::MED(abj::String correction, abj::String typo){ //(newWord, mainWord)
  this->initialize_matrix();
  std::string correctionSTD;
  for(int i=0; i<correction.size(); i++){
    correctionSTD.append(1, correction[i]);
  }
  this->correction = correctionSTD;

  std::string typoSTD;
  for(int i=0; i<typo.size(); i++){
    typoSTD.append(1, typo[i]);
  }
  this->typo = typoSTD;
}

void MED::initialize_matrix(){
...
int MED::getDistance(){
  return m_distance[correction.size()-1][typo.size()-1];
}
